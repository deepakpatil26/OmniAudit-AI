...
export default function App() {
  ...
  // Logic Hooks
  const { transcript, aiResponse, isConsultantThinking } = useVoiceConsult(showLiveConsult);
  ...
  const filteredAudits = useMemo(() => {
    ...
    return audits.filter(audit => audit.findings.some(f => f.status === 'discrepancy'));
  }, [audits, filter]);

  // Insight Stats
  const stats = useMemo(() => {
    const highRiskCount = audits.filter(a => a.findings.some(f => f.status === 'discrepancy')).length;
    const avgScore = audits.length > 0 
      ? Math.round(audits.reduce((acc, curr) => acc + (curr.complianceScore || 0), 0) / audits.length) 
      : 100;
  ...
  return (
    <div className='min-h-screen bg-theme-secondary text-text-primary transition-colors duration-300'>
      ...
      <AnimatePresence mode='wait'>
        {view === 'landing' && (
          <motion.div key='landing' initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <Landing onStartAudit={handleStartAuditClick} isLoggedIn={!!user} />
          </motion.div>
        )}
        ...
        {view === 'dashboard' && user && (
          <motion.main 
            ...
            <div className='grid grid-cols-1 md:grid-cols-3 gap-6 mb-12'>
               <div className='bg-theme-primary p-6 rounded-[2rem] border border-border-primary shadow-sm'>
                  <div className='flex items-center gap-4'>
                     ...
                     <div>
                        <div className='text-xs font-bold text-text-secondary uppercase tracking-widest'>Risk detections</div>
                        <div className='text-3xl font-bold text-text-primary leading-none mt-1'>{stats.highRiskCount}</div>
                     </div>
                  </div>
               </div>
               <div className='bg-theme-primary p-6 rounded-[2rem] border border-border-primary shadow-sm'>
                  <div className='flex items-center gap-4'>
                     ...
                     <div>
                        <div className='text-xs font-bold text-text-secondary uppercase tracking-widest'>Integrity Score</div>
                        <div className='text-3xl font-bold text-text-primary leading-none mt-1'>{stats.avgScore}%</div>
                     </div>
                  </div>
               </div>
               <div className='bg-theme-primary p-6 rounded-[2rem] border border-border-primary shadow-sm'>
                  <div className='flex items-center gap-4'>
                     ...
                     <div>
                        <div className='text-xs font-bold text-text-secondary uppercase tracking-widest'>Market Coverage</div>
                        <div className='text-3xl font-bold text-text-primary leading-none mt-1'>{stats.regions} Regions</div>
                     </div>
                  </div>
               </div>
            </div>
            ...
            <div className='flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4'>
              <div>
                <h2 className='text-2xl font-bold text-text-primary tracking-tight leading-none'>Expert Ledger</h2>
                <p className='text-sm text-text-secondary font-medium italic mt-2'>Manage your statutory compliance reports.</p>
              </div>
              <div className='flex items-center gap-2 bg-theme-primary p-1 rounded-xl border border-border-primary shadow-sm'>
                <button
                  ...
                  className={`px-4 py-1.5 text-[10px] font-bold uppercase tracking-widest rounded-lg transition-all ${
                    filter === 'all' ? 'bg-theme-secondary text-text-primary' : 'text-text-secondary hover:text-text-primary'
                  }`}>
                  All Records
                </button>
                <button
                  ...
                  className={`px-4 py-1.5 text-[10px] font-bold uppercase tracking-widest rounded-lg transition-all ${
                    filter === 'high-risk' ? 'bg-red-50 dark:bg-red-900/40 text-red-600' : 'text-text-secondary hover:text-text-primary'
                  }`}>
                  High Risk
                </button>
              </div>
            ...
            {filteredAudits.length === 0 ? (
              <div className='bg-white dark:bg-gray-800/30 border-2 border-dashed border-gray-100 dark:border-gray-800 rounded-[2.5rem] p-16 text-center'>
                ...
                <h3 className='text-xl font-bold text-gray-900 dark:text-white mb-2 uppercase tracking-tight'>
                  {filter === 'all' ? 'Statutory Ledger Empty' : 'No Critical Deviations Found'}
                </h3>
              ...
              <div className='grid grid-cols-1 lg:grid-cols-2 gap-8'>
                {filteredAudits.map((audit) => (
                  <AuditCard key={audit.id} audit={audit} onViewReport={setSelectedAudit} onDelete={handleDeleteAudit} />
                ))}
              </div>
            )}
          </motion.main>
      ...
      <UploadModal show={showUploadModal} onClose={() => setShowUploadModal(false)} {...form} />
      <ReportModal audit={selectedAudit} onClose={() => setSelectedAudit(null)} onGeneratePDF={generateAuditPDF} onFixImage={handleFixImage} isFixing={isFixing} />
      <LiveConsultOverlay show={showLiveConsult} onClose={() => setShowLiveConsult(false)} transcript={transcript} aiResponse={aiResponse} isConsultantThinking={isConsultantThinking} />
    </div>
  );
}
